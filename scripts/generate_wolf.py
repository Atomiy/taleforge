...
with open(r'scripts\wolf_config.json', 'r', encoding='utf-8') as f:
    config = json.load(f)
...
def generate_chapter(ch, prev_id=None):
    ...
    out_path = f"scripts/wolf_ch{ch['chapter_num']}.txt"
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(f"## 第{ch['chapter_num']}章 {story_title or ch['title']}\n\n{full_content}")
    print(f"  已备份: {out_path} ({len(full_content)}字)")
...
def main():
    ...
    for ch in config['chapters']:
        ...
        if sid:
            prev_id = sid
            results.append((ch['chapter_num'], title, len(content) if content else 0))
        else:
            print(f"  第{ch['chapter_num']}章生成失败，终止")
            break

    print(f"\n{'='*60}")
    print("  苍狼传说 第一卷 生成完成")
    for num, title, wc in results:
        print(f"  第{num}章「{title}」: {wc}字")
    print(f"{'='*60}")
