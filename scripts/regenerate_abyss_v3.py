"""
深渊回响 v3 - 生成脚本 (Character Agent + Writer 双约束版本)
用于对比验证：v0(无出场表) vs v2(仅Writer约束) vs v3(Character+Writer双约束)
"""
import json
import uuid
import os
    ...
    "volumes": [
        {
            ...
            "chapter_ids": [],
            "outline": (
                "第1章「寂静深海」：深渊计划组建完毕，主角团队乘深海探测器下潜至马里亚纳海沟8000米深处，"
                "首次接触深渊意识体，团队内部因对未知的态度分歧产生裂痕。"
                "第2章「符号密语」：科学家发现结构体上的符号与古苏美尔文字存在部分对应关系，"
                "翻译出一句警告——\"不要让它完整地听见你\"。"
                "第3章「裂痕」：深渊意识体对不同队员展现出不同的\"面孔\"，"
                "有人听到已故亲人的呼唤，有人听到未来灾难的预言。"
                "团队分裂为相信派与怀疑派，怀疑派试图摧毁精神链接装置。"
            ),
...
config = {
    ...
    "work_title": "深渊回响 v3 (双重约束验证)",
    "genre": "科幻",
    "style": "克苏鲁+硬科幻",
    "world_setting": new_work["world_setting"],
    "foreshadowings": new_work["foreshadowings"],
    "characters": [
        {"name": "林深", "age": "35", "identity": "深海地质学家", "appearance": "中等身材，深邃的眼神", "personality": "理性、冷静、执着"},
        {"name": "陈霜", "age": "32", "identity": "海军深海潜航员", "appearance": "身材精悍，眼神锐利", "personality": "警惕、果敢、寡言"},
        {"name": "Dr.索菲娅·米兰", "age": "40", "identity": "符号学家", "appearance": "金发盘起，戴细框眼镜", "personality": "谨慎、细致、善于调停"},
...
config_path = r'scripts\abyss_v3_config.json'
with open(config_path, 'w', encoding='utf-8') as f:
    json.dump(config, f, ensure_ascii=False, indent=2)

print(f"[OK] 已创建新作品: 深渊回响 v3 (双重约束验证)")
