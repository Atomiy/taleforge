"""
v3.4 无矛盾剧情设计版
核心改进：Ch2 剧情驱动力从"赵海脑波数据"改为"深渊之眼主动发送符号信号"
"""
import json, uuid
...
# ===== 角色弧线（无矛盾措辞） =====
character_arcs = {
    "林深": {1: "初探深渊，科学狂热", 2: "主导研究符号，坚信可破译", 3: "发现封印真相，选择坚守"},
    "陈霜": {1: "极度警惕，主张限制链接", 2: "监控设备异常，立场动摇", 3: "决意摧毁装置，与林深对立"},
    "Dr.索菲娅·米兰": {1: "中立观察，记录数据", 2: "破译符号，发现动态变化", 3: "理解封印本质，成为调解者"},
    "赵海": {1: "正常操作通讯设备，初次链接异常",
             2: "精神被侵蚀，处于深度昏迷——仅作为背景事实被提及",
             3: "完全被深渊占据，成为传声筒"},
    "深渊之眼": {1: "以发光符号初次接触",
                 2: "通过结构体符号主动传播信号，对每个人展现不同信息",
                 3: "展露真面目，试图突破封印"},
}
...
config = {
    ...
    "chapters": [
        ...
        # Ch2 key_events 不依赖赵海——剧情由符号信号驱动
        {"chapter_num": 2, "title": "符号密语",
...
with open(r'scripts\abyss_v34_config.json', 'w', encoding='utf-8') as f:
    json.dump(config, f, ensure_ascii=False, indent=2)

print(f"[OK] 深渊回响 v3.4 (无矛盾剧情设计版)")
